<?php 
  ...
  if (isset($_POST['btnLogin'])) {
  	...
  	$checkUsername = mysqli_query($koneksi, "SELECT * FROM tb_user WHERE username = '$username' AND password='$password'");
  	if ($data = mysqli_fetch_assoc($checkUsername)) {
		$cek=mysqli_num_rows($checkUsername);
  		if($cek>0){ 
  			$_SESSION = [
		...
		<div style="background-color: transparent;" class="container-fluid mt-5">
			<div class="row justify-content-center">
				<div class="col-lg text-center text-white pt-4 pb-2">
					<p>&copy; Copyright 2024. By Al Hikmah. All Right Reserved.</p>
				</div>
			</div>
		</div>
	</footer>
